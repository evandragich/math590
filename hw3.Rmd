...
The survival function $S_{x}(t)$ is defined as the complement of the distribution function $F_{x}t$. Thus, we can simply subtract from 1 to arrive at  $S_{x}(t)$
...
No, I do not believe this is reasonable to model mortality. The associated survival function initially sharply decreases, before gradually levelling off as age approaches infinity. This is due to the constate force of mortality $\mu_{x} = \lambda$.

However, this is contrary to the phenomenon of human mortality, where death does not begin to pick up at a significant rate until later in life. Thus, we would want to choose a survival function that more appropriately models the force of mortality as increasing over age, and thus would include $x$ in the expression for $\mu_{x}$ such as the Gompertz or Makeham.
...
If we evaluate the derivative using the chain rule, we see that the derivative of $\px[t]{x}$ at a certain $x$ is scaled by a factor of $(\mu_{x} - \mu_{x + t} )$. Writing these concepts as such provides more insight on the role of the force of mortality, $\mu$, as the negative, scaled, instantaneous rate of change in the survival function $S_{0}$ at a particular $x$.
